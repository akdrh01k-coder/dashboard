...
st.set_page_config(
    page_title="ECO-friendSHIP",
    page_icon="🚢",
    layout="wide",        
)
st.markdown("""
<style>
  .block-container {max-width: 1800px; padding-left: 2rem; padding-right: 2rem;}
...
st.caption("친환경 수소 선박 시뮬레이터 • 실시간 모니터링 및 제어")
...
with center_col:
  ...
  # ---- robust slideshow (no index access) ----
import os, base64
from streamlit.components.v1 import html
...
# 프로젝트/파일 기준 안전 경로(assets 폴더에 이미지를 두는 걸 권장)
BASE_DIR = os.path.dirname(__file__)
CANDIDATE_PATHS = [
    os.path.join(BASE_DIR, "your_ship_image.png"),
    os.path.join(BASE_DIR, "image1.png"),
    os.path.join(BASE_DIR, "image2.png"),
    os.path.join(BASE_DIR, "image3.png"),
]

image_sources = []
for p in CANDIDATE_PATHS:
    if os.path.exists(p):
        ext = (os.path.splitext(p)[1][1:] or "png").lower()
        image_sources.append(f"data:image/{ext};base64,{_b64(p)}")

# 폴백(로컬 이미지가 하나도 없을 때)
if not image_sources:
    image_sources = [
        "https://picsum.photos/seed/ship1/1000/450",
        "https://picsum.photos/seed/ship2/1000/450",
        "https://picsum.photos/seed/ship3/1000/450",
    ]

slides_html = "\n".join(
    f'<div class="mySlides fade"><img src="{src}"></div>'
    for src in image_sources
)

slideshow_html = f"""
<style>
  .slideshow-container {{ max-width: 1000px; position: relative; margin: auto; border-radius: 8px; overflow: hidden; }}
